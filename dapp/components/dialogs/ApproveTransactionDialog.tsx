...
export function ApproveTransactionDialog({
  ...
  useEffect(() => {
    ...
    const fetchTransactionBytes = async () => {
      ...
      try {
        ...
        const tx = Transaction.from(decodedBytes);
        setTransaction(tx);
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      tx.moveCall({
        target: `${PACKAGE_ID}::dynamic_auth::approve_transaction`,
        arguments: [
          tx.object(isafeAccount),
          tx.pure(bcs.vector(bcs.u8()).serialize(fromBase58(txDigest))),
        ],
      });
      ...
      signAndExecuteTransaction(
        ...
        },
        {
          ...
          onError: (error) => {
            setApprovalError(`Failed to approve transaction: ${error.message}`);
          },
        }
      );
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : String(err);
      setApprovalError(`Failed to approve transaction: ${message}`);
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-background border border-foreground/20 rounded-xl shadow-2xl w-full max-w-2xl mx-4">
            ...
            <div className="space-y-6">
              ...
              {approvalError && (
                <div className="flex items-start gap-2 p-3 bg-red-500/10 border border-red-500/20 rounded-lg">
                  ...
                  <p className="text-sm text-red-500">{approvalError}</p>
                </div>
              )}
            </div>
          )}
        ...
        {/* Footer */}
        {!approvalSuccess && !isLoading && !loadError && (
          <div className="flex justify-end gap-3 p-6 border-t border-foreground/10">
            ...
            <button
              ...
              disabled={!simulationPassed || isPending}
              className="px-4 py-2 text-sm font-medium bg-green-600 text-white rounded-lg hover:bg-green-700 transition disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
            >
              {isPending ? (
                <>
                  <svg
