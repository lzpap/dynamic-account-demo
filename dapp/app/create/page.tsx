...
function isValidIotaAddress(address: string): boolean {
    // IOTA addresses are 32 bytes hex strings with 0x prefix (66 characters total)
...
export default function Create() {
    ...
    const addCurrentWallet = () => {
        if (currentAccount?.address && !members.some(m => m.address === currentAccount.address)) {
            setMembers([...members, { address: currentAccount.address, weight: '1' }]);
        }
    };
    ...
    const validateForm = (): { valid: boolean; error: string } => {
        ...
        for (const member of members) {
            ...
            if (!isValidIotaAddress(member.address)) {
                return { valid: false, error: `Invalid IOTA address: ${member.address}` };
            }
        }
        ...
        for (const member of members) {
            ...
            if (isNaN(weight) || weight <= 0) {
                return { valid: false, error: 'All member weights must be positive numbers' };
            }
        }

        // Calculate total weight
        const totalWeight = members.reduce((sum, m) => sum + parseInt(m.weight, 10), 0);
        ...
        if (thresholdNum > totalWeight) {
            return { valid: false, error: `Threshold (${thresholdNum}) cannot exceed total weight (${totalWeight})` };
        }
            ...
            signAndExecuteTransaction(
                ...
                },
                {
                    onSuccess: (result) => {
                        ...
                        setSuccess(`Account created successfully! Digest: ${result.digest}`);
                        // Reset form
                        setMembers([{ address: currentAccount?.address || '', weight: '1' }]);
                        setThreshold('1');
                    },
                    onError: (error) => {
    ...
    return (
        <main className="flex flex-col min-h-screen ml-64 p-8">
            <h1 className="text-4xl font-bold text-center mt-10 mb-8">Create A New iSafe Account</h1>
            
            <form onSubmit={handleSubmit} className="max-w-2xl mx-auto w-full bg-foreground/5 rounded-lg shadow-md p-6">
                {/* Members Section */}
                <div className="mb-6">
                ...
                {/* Threshold Section */}
                <div className="mb-6">
                    <label htmlFor="threshold" className="block text-foreground font-semibold text-lg mb-2">
                        Threshold
                    </label>
                    <input
                        type="number"
            ...
            <div className="max-w-2xl mx-auto w-full mt-6 p-4 bg-foreground/5 rounded-lg">
                ...
                <ul className="text-sm text-foreground/80 space-y-1 list-disc list-inside">
                    ...
                    <li>Transactions require approval from members whose combined weight meets the threshold</li>
